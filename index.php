...
{
    return count(data_load('replies_' . $threadId . '.json'));
}

function threadLikeCount(int $threadId): int
{
    return count(data_load('likes_thread_' . $threadId . '.json'));
}
...
function threadDate(array $thread): string
{
    ...
    return $date === '' ? 'Дата неизвестна' : $date;
}

function forumAvatar(array $user): string
{
    $avatar = trim((string)($user['avatar'] ?? ''));
    return $avatar !== '' ? $avatar : 'banners/IMG_20260727_215431_065.jpg';
}

function forumProfileUrl(array $user): string
{
    $handle = trim((string)($user['handle'] ?? ''));

    if ($handle !== '') {
        return SITE_URL . '/' . rawurlencode($handle);
    }

    return 'profile.php?id=' . (int)($user['id'] ?? 0);
}
...
.topic-avatar-premium {
    border-color: var(--accent, #ff7620);
}
...
<?php if (!empty($banner['enabled'])): ?>
    <section class="billboard">
        ...
        <?php if (!empty($banner['title'])): ?>
            <h2><?= e((string)$banner['title']) ?></h2>
        <?php endif; ?>

        <?php if (!empty($banner['text'])): ?>
            <p><?= nl2br(e((string)$banner['text'])) ?></p>
        <?php endif; ?>
...
<section class="forum-section">
    <div class="section-heading">
        ...
        <a href="search.php" class="section-link">Найти тему →</a>
    </div>

    <?php if (!$categories): ?>
        <div class="card muted">Разделов пока нет.</div>
    <?php else: ?>
        <div class="category-list">
            <?php foreach ($categories as $category): ?>
                ...
                $categoryCount = categoryThreadCount($threads, $categoryId);
                ?>
                ...
                <article class="card category-card">
                    ...
                    <div class="category-content">
                        ...
                        <?php if ($categoryDescription !== ''): ?>
                            <p class="muted"><?= e($categoryDescription) ?></p>
                        <?php endif; ?>
                    </div>
...
<section class="forum-section">
    <div class="section-heading">
        ...
        <?php if ($currentUser): ?>
            <a href="create-thread.php" class="btn small">Новая тема</a>
        <?php endif; ?>
    </div>
    ...
    <?php else: ?>
        <div class="thread-list">
            <?php foreach ($latestThreads as $thread): ?>
                ...
                $threadTitle = (string)($thread['title'] ?? 'Без названия');
                $threadAuthor = (string)($thread['author'] ?? 'Неизвестный');
                ...
                $isPremium = $authorId > 0 && premium($author);
                ?>

                <article class="card thread-card">
                    <div class="thread-main">
                        <?php if ($pinned): ?>
                            <div class="pin">📌 Закреплено владельцем</div>
                        <?php endif; ?>
                        ...
                        <div class="thread-meta">
                            <a
                                class="topic-author"
                                href="<?= e(forumProfileUrl($author)) ?>"
                            >
                                <img
                                    class="topic-avatar <?= $isPremium ? 'topic-avatar-premium' : '' ?>"
                                    src="<?= e(forumAvatar($author)) ?>"
                                    alt=""
                                    loading="lazy"
                                >

                                <span
                                    class="thread-author-name"
                                    style="<?= $isPremium ? 'color:var(--accent,#ff7620);' : '' ?>"
                                >
                                    <?= e((string)($author['username'] ?? $threadAuthor)) ?>
                                </span>
...
<section class="forum-section recommendations-preview">
    <div class="section-heading">
        ...
        <a href="recommendations.php" class="section-link">Все рекомендации →</a>
    </div>

    <?php if (!$recommendations): ?>
        <div class="card muted">Рекомендаций пока нет.</div>
    <?php else: ?>
        <div class="recommendation-list">
            <?php foreach ($recommendations as $recommendation): ?>
                <?php
                $recommendationTitle = (string)($recommendation['title'] ?? 'Рекомендация');
                $recommendationText = (string)($recommendation['text'] ?? '');
                $recommendationUrl = (string)($recommendation['url'] ?? '#');
                $recommendationPinned = !empty($recommendation['pinned']);
                $recommendationScore = (int)($recommendation['score'] ?? 0);
                ?>
                ...
                <article class="card recommendation-card">
                    ...
                    <div class="recommendation-body">
                        <?php if ($recommendationPinned): ?>
                            <span class="pin-label">Закреплено владельцем</span>
                        <?php endif; ?>
                        ...
                        <?php if ($recommendationText !== ''): ?>
                            <p class="muted"><?= e($recommendationText) ?></p>
                        <?php endif; ?>
                    </div>
