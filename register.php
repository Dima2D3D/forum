...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($users as $x) {
        if (strtolower((string)($x['email'] ?? '')) === $email || strtolower((string)($x['username'] ?? '')) === strtolower($name)) {
    ...
    if (!$err) {
        ...
        $now = date('c');
        $role = strtolower($email) === strtolower(OWNER_EMAIL) ? 'owner' : 'user';
        $users[] = [
            'id' => $id,
            'username' => $name,
            'handle' => $name,
        ...
        if (mail_verification($email, $name, $link)) {
            $ok = 'Аккаунт создан. Письмо с кнопкой подтверждения отправлено на E-mail.';
        } else {
            $ok = 'Аккаунт создан, но хостинг не смог отправить письмо. Проверьте настройки почты.';
        }
    }
}
...
<div class="card form">
    ...
    <form method="post">
        ...
        <small class="muted">После регистрации он будет отображаться как @юзернейм. Изменить его можно в настройках профиля.</small>
        <label>E-mail</label>
