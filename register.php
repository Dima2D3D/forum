...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $reserved = ['admin','login','register','settings','messages','message','notifications','search','index','profile','rules','privacy','offer','gifts','achievements','banned','forgot-password','reset-password'];
    ...
    if (!$err) {
        ...
        try {
            ...
            if (mail_verification($email, $name, $link)) {
                ...
                $ok = 'Аккаунт создан, но письмо не удалось отправить. Попробуйте запросить письмо позже или обратитесь к администратору.';
            }
        } catch (Throwable $e) {
