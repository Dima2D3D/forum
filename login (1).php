...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!rate_limit('login', 30, 5)) {
        ...
        if (!$found || !password_verify($pass, (string)($found['password_hash'] ?? ''))) {
            ...
            header('Location: banned.php', true, 303);
            exit;
            ...
            try {
                ...
                session_regenerate_id(true);
                unset($_SESSION['uid'], $_SESSION['activity_touch']);
                ...
                if (!send_html_mail((string)$found['email'], 'Код входа — GREFFRLEND', $html, "Ваш код входа: $code. Код действует 10 минут.")) {
                    ...
                    $err = 'Не удалось отправить код 2FA. Попробуйте позже.';
                } else {
                    session_write_close();
                    header('Location: ' . SITE_URL . '/2fa.php', true, 303);
                    exit;
                }
            } catch (Throwable $e) {
                unset($_SESSION['2fa_pending'], $_SESSION['2fa_uid'], $_SESSION['2fa_code_hash'], $_SESSION['2fa_expires']);
