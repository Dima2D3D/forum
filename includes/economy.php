...
function change_wallet(int $userId, int $delta, string $reason, string $actor = 'system'): bool {
    ...
    $logs[] = ['id' => next_id($logs), 'user_id' => $userId, 'delta' => $delta, 'balance' => $new, 'reason' => $reason, 'actor' => $actor, 'time' => date('c'), 'username' => $name];
...
function gifts(): array {
    return data_load('gifts.json', [
        ...
        ['id' => 2, 'name' => 'Сердце', 'emoji' => '❤️', 'price' => 100, 'enabled' => true],
        ['id' => 3, 'name' => 'Алмаз', 'emoji' => '💎', 'price' => 500, 'enabled' => true],
        ['id' => 4, 'name' => 'Корона', 'emoji' => '👑', 'price' => 1000, 'enabled' => true],
...
function give_gift(int $from, int $to, int $giftId, string $description = ''): bool {
    foreach (gifts() as $g) {
        ...
        if (!change_wallet($from, -(int)$g['price'], 'Подарок: ' . $g['name'], 'user:' . $from)) return false;
