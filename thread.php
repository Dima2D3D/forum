...
function render_reply(array $reply, array $replies, ?array $me, int $threadId, int $level = 0): void
{
    ...
    <article class="card post comment" id="reply-<?= $replyId ?>" style="margin-left:<?= $level * 24 ?>px">
        <div class="post-meta">
            <a href="profile.php?id=<?= (int)($reply['author_id'] ?? 0) ?>">
                <b><?= e($reply['author'] ?? 'Пользователь') ?></b>
            </a>
        ...
        <div class="reactions">
            <?php if ($me): ?>
                ...
                <button class="reaction" type="button" onclick="replyTo(<?= $replyId ?>, '<?= e($reply['author'] ?? '') ?>')">
                    ↩ Ответить
                </button>
            <?php endif; ?>

            <?php if ($canEdit): ?>
                <button class="reaction" type="button" onclick="toggleEdit('edit-reply-<?= $replyId ?>')">
                    ✏️ Изменить
                </button>
                ...
                <form method="post" action="content_action.php" class="inline-form">
                    ...
                    <button class="reaction" type="submit">🗑️ Удалить</button>
                </form>
            <?php endif; ?>
        </div>

        <?php if ($canEdit): ?>
            <form id="edit-reply-<?= $replyId ?>" class="card form edit-form" style="display:none" method="post" action="content_action.php">
                <input type="hidden" name="csrf" value="<?= e(csrf()) ?>">
                <input type="hidden" name="type" value="reply">
                <input type="hidden" name="thread_id" value="<?= $threadId ?>">
                <input type="hidden" name="reply_id" value="<?= $replyId ?>">
                ...
                <textarea name="message" maxlength="20000" required><?= e((string)($reply['message'] ?? '')) ?></textarea>

                <button class="btn" type="submit">💾 Сохранить</button>
                <button class="reaction" type="button" onclick="toggleEdit('edit-reply-<?= $replyId ?>')">Отмена</button>
            </form>
        <?php endif; ?>
...
$canEditThread = $me && ($threadAuthorId === (int)$me['id'] || is_owner($me));
$canManageThread = $me && is_owner($me);
?>
...
<article class="card post" id="post-<?= $id ?>">
    ...
    <div class="post-meta">
        <a href="profile.php?id=<?= $threadAuthorId ?>">
            <b><?= e($thread['author'] ?? '') ?></b>
        </a>
    ...
    <div class="reactions">
        ...
        <?php if ($canEditThread): ?>
            <button class="reaction" type="button" onclick="toggleEdit('edit-post')">
                ✏️ Изменить пост
            </button>
        <?php endif; ?>

        <?php if ($canManageThread): ?>
            <form method="post" action="content_action.php" class="inline-form" onsubmit="return confirm('Удалить этот пост?')">
                <input type="hidden" name="csrf" value="<?= e(csrf()) ?>">
                <input type="hidden" name="type" value="thread">
                <input type="hidden" name="id" value="<?= $id ?>">
                <input type="hidden" name="action" value="delete">
            ...
            <form method="post" action="content_action.php" class="inline-form">
                ...
                <input type="hidden" name="type" value="thread">
                <input type="hidden" name="id" value="<?= $id ?>">
                <input type="hidden" name="action" value="<?= !empty($thread['pinned']) ? 'unpin' : 'pin' ?>">
                <button class="reaction" type="submit">
                    📌 <?= !empty($thread['pinned']) ? 'Открепить' : 'Закрепить' ?>
                </button>
            </form>
        <?php endif; ?>
    </div>
    ...
    <?php if ($canEditThread): ?>
        <form id="edit-post" class="card form edit-form" style="display:none" method="post" action="content_action.php">
            <input type="hidden" name="csrf" value="<?= e(csrf()) ?>">
            <input type="hidden" name="type" value="thread">
            <input type="hidden" name="id" value="<?= $id ?>">
            <input type="hidden" name="action" value="edit">
            ...
            <input name="title" maxlength="120" value="<?= e($thread['title'] ?? '') ?>" required>

            <label>Текст поста</label>
            <textarea name="content" maxlength="20000" required><?= e($thread['content'] ?? '') ?></textarea>

            <button class="btn" type="submit">💾 Сохранить изменения</button>
            <button class="reaction" type="button" onclick="toggleEdit('edit-post')">Отмена</button>
        </form>
    <?php endif; ?>
</article>
    ...
    <div class="card form" id="reply-form">
        ...
        <div class="emoji">
            <?php foreach (['😀','😂','❤️','🔥','👍','👎','🎮','😎','🤔','🎉','💀','🚀','🥳','😢','😡'] as $emoji): ?>
                <button type="button" onclick="insertEmoji('<?= e($emoji) ?>')">
                    <?= e($emoji) ?>
                </button>
            <?php endforeach; ?>
        </div>
...
<?php else: ?>
    <div class="card">
        Чтобы отвечать, <a href="login.php">войдите</a> или <a href="register.php">зарегистрируйтесь</a>.
    </div>
<?php endif; ?>
