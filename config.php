...
const PROFILE_DESCRIPTION_LINES = 10;
...
function is_owner(?array $u = null): bool {
    $u ??= current_user();
    return $u && (($u['role'] ?? '') === 'owner' || strtolower((string)($u['email'] ?? '')) === strtolower(OWNER_EMAIL));
}

function is_admin(?array $u = null): bool {
    $u ??= current_user();
    return is_owner($u) || ($u && ($u['role'] ?? '') === 'admin');
}
...
function banned_user(?array $u = null): ?array {
    $u ??= current_user();
    if ($u && !empty($u['ban']['active'])) return $u['ban'];
    $ip = client_ip();
    foreach (data_load('bans.json') as $b) {
        if (($b['type'] ?? '') !== 'ip') continue;
        if (!hash_equals((string)($b['value'] ?? ''), $ip)) continue;
        $until = $b['until'] ?? 'permanent';
        if ($until === 'permanent' || strtotime((string)$until) > time()) return $b;
    }
    return null;
}
...
function require_owner(): array {
    ...
    if (!is_owner($u)) {
        http_response_code(403);
        exit('Только владелец форума может выполнить это действие.');
    }
    return $u;
}

function log_action(string $action, string $target, string $reason = ''): void {
    $logs = data_load('logs.json');
    $logs[] = [
        'id' => next_id($logs),
        'action' => $action,
        'target' => $target,
    ...
    data_save('logs.json', $logs);
}

function rate_limit(string $key, int $seconds = 15, int $max = 1): bool {
    $now = time();
    $bucket = $_SESSION['rate'] ?? [];
    $bucket[$key] = array_values(array_filter($bucket[$key] ?? [], fn($t) => $t > $now - $seconds));
    if (count($bucket[$key]) >= $max) {
...
function clean_text(string $text, int $max = 20000): string {
    $text = trim($text);
    $text = preg_replace('/\x00/u', '', $text) ?? '';
    return mb_substr($text, 0, $max);
}

function profile_description(string $text, bool $owner = false): string {
    $text = clean_text(str_replace(["\r\n", "\r"], "\n", $text), $owner ? 20000 : PROFILE_DESCRIPTION_LIMIT);
    if ($owner) return $text;
    $lines = preg_split('/\n/', $text) ?: [];
    return implode("\n", array_slice($lines, 0, PROFILE_DESCRIPTION_LINES));
}

function allowed_upload(string $tmp, string $name): bool {
    if (!is_uploaded_file($tmp) || !is_file($tmp) || filesize($tmp) > MAX_UPLOAD_BYTES) return false;
    $mime = (new finfo(FILEINFO_MIME_TYPE))->file($tmp);
    return in_array($mime, ['image/jpeg', 'image/png', 'image/gif', 'image/webp'], true);
}

function save_single_upload(string $field): ?string {
    if (empty($_FILES[$field]['tmp_name']) || !is_string($_FILES[$field]['tmp_name'])) return null;
    $tmp = $_FILES[$field]['tmp_name'];
    $name = (string)($_FILES[$field]['name'] ?? '');
    if (!allowed_upload($tmp, $name)) return null;
    $mime = (new finfo(FILEINFO_MIME_TYPE))->file($tmp);
    $ext = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/gif' => 'gif', 'image/webp' => 'webp'][$mime] ?? null;
    if (!$ext) return null;
    $file = bin2hex(random_bytes(16)) . '.' . $ext;
    return move_uploaded_file($tmp, UPLOAD_DIR . $file) ? 'uploads/' . $file : null;
}

function save_uploads(string $field = 'attachments'): array {
    if (empty($_FILES[$field]['name']) || !is_array($_FILES[$field]['name'])) return [];
    $saved = [];
    $count = min(count($_FILES[$field]['name']), MAX_ATTACHMENTS);
    for ($i = 0; $i < $count; $i++) {
        $tmp = $_FILES[$field]['tmp_name'][$i] ?? '';
        $name = $_FILES[$field]['name'][$i] ?? '';
        if (!allowed_upload($tmp, $name)) continue;
        $mime = (new finfo(FILEINFO_MIME_TYPE))->file($tmp);
        $ext = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/gif' => 'gif', 'image/webp' => 'webp'][$mime] ?? null;
        if (!$ext) continue;
        $file = bin2hex(random_bytes(16)) . '.' . $ext;
        if (move_uploaded_file($tmp, UPLOAD_DIR . $file)) $saved[] = 'uploads/' . $file;
    }
    return $saved;
}

function send_html_mail(string $to, string $subject, string $html, string $text = ''): bool {
    $boundary = '=_greffrlend_' . bin2hex(random_bytes(8));
...
function mail_verification(string $email, string $username, string $url): bool {
    ...
    return send_html_mail($email, 'Подтверждение E-mail — GREFFRLEND', $html, "Привет, $username! Подтвердите E-mail: $url");
}

if (($ban = banned_user()) && basename($_SERVER['SCRIPT_NAME'] ?? '') !== 'banned.php') {
    header('Location: banned.php');
    exit;
}
