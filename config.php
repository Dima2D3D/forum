...
function is_owner(?array $u = null): bool { $u ??= current_user(); return $u && (($u['role'] ?? '') === 'owner' || strtolower((string)($u['email'] ?? '')) === strtolower(OWNER_EMAIL)); }
...
function banned_user(?array $u = null): ?array {
    ...
    foreach (data_load('bans.json') as $b) {
        if (($b['type'] ?? '') !== 'ip' || !hash_equals((string)($b['value'] ?? ''), $ip)) continue;
...
function profile_description(string $text, bool $owner = false): string { $text=clean_text(str_replace(["\r\n","\r"],"\n",$text),$owner?20000:PROFILE_DESCRIPTION_LIMIT);$lines=preg_split('/\n/',$text)?:[];return $owner?$text:implode("\n",array_slice($lines,0,PROFILE_DESCRIPTION_LINES)); }
